...
### 从源码构建并使用 Docker Compose

如果你希望使用自己修改后的代码（而不是官方镜像），可以按以下步骤操作。
...
<Steps>
  ...
  ```sh
  docker build -t lobehub/lobe-chat:local .
  ```

  ### 使用部署模板并改为本地镜像

  ```sh
  cp docker-compose/deploy/docker-compose.yml ./docker-compose.yml
  cp docker-compose/deploy/.env.zh-CN.example ./.env
  ```

  打开 `docker-compose.yml`，将 `lobehub` 服务里的镜像改为你本地构建的镜像：

  ```yaml
  services:
    lobehub:
      image: lobehub/lobe-chat:local
  ```

  如果模板里存在 `build:` 字段，请删除或注释掉它，避免与 `image:` 同时生效造成混淆。
